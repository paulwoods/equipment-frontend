...
export const StatusBadge = ({status}: { status: Equipment["status"] }): React.JSX.Element => {
    return (
        <span
            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${statusColors[status] || statusColors.Active}`}
        >
            {status}
        </span>
    );
